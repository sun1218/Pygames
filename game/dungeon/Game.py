...
from Pygames.game.dungeon.Dungeon import Dungeon
from Pygames.game.dungeon.Player import Player, Monster
from Pygames.game.dungeon.library import print_text
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == K_ESCAPE:
                ...
                if not player.moveUp():
                    playerCollision(0, -1)
                else:
                    move_monsters()

            elif event.key == K_DOWN or event.key == K_s:
                if not player.moveDown():
                    playerCollision(0, 1)
                else:
                    move_monsters()

            elif event.key == K_RIGHT or event.key == K_d:
                if not player.moveRight():
                    playerCollision(1, 0)
                else:
                    move_monsters()

            elif event.key == K_LEFT or event.key == K_a:
                if not player.moveLeft():
                    playerCollision(-1, 0)
                else:
                    move_monsters()
